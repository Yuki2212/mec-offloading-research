...
このフォルダは、MECタスクオフロードの比較実験・解析・生成物をまとめた研究用リポジトリです。
...
`requirements.txt` は依存ライブラリの範囲指定です。特定の実行環境を固定したファイルではありません。
...
4冊は各13セルで構成し、CSV読込・描画・集計は `analysis/reporting.py` を共有しています。CSVは一度だけ読み込みます。
...
基準CSVは保存済みの実験結果です。通常実験を実行すると、実行時間は環境によって変わります。
環境生成seedとGA設定のseedは別です。Greedyの初期解とRandomの割当は `np.random.default_rng()` を使うため、同じ設定でも毎回同一の結果になるとは限りません。
...
この点を踏まえ、研究上の修正を行う場合は基準結果との差を検証してください。
